...
2. Compare them for $n = 100, 200, 500$, adding a vertical line to represent the $h_{\text{MISE}}$ and $h_{\text{AMISE}}$ bandwidths. Describe in detail the results and the major takeaways.
...
Then, we develop some assymptotic expressions for (1) and (2):
...
We define the dominating part of equation (9) as $\text{AMISE}[\hat{f}(\cdot;h)]$. In particular:
...
(12) $h_{\text{AMISE}} = {\left[ {\frac{R(K)}{\mu_{2}^{2}(K)R(f^{''})n}} \right]}^{1/5}$
...
With these two expressions, we can obtain a specific value for the AMISE in equation (10). With assumption a), we obtain the following expressions for equations (6) and (7)

(6.1) Second order moment of $K$: $\mu_{2}(K) =  1$

(7.1) Squared integral of kernel: $R(K) =  \frac{1}{2 \sqrt{\pi}}$

Expression for equation (11) can be obtained from the following adaption of expression given in *Theorem 4.1*  given by *Marron and Wand (1992)* (In particular, taking $s = 2$ in the expression given in the paper)

(11.1) $R(f^{''}) = \int {(f^{''}(x))}^{2} dx = \sum_{j=1}^{r} \sum_{j'= 1}^{r} w_{j} w_{j'} \phi_{\sigma_{jj'}}^{4}(\mu_{j} - \mu_{j'})$

where $\sigma_{jj'} = {\sigma_{j}^{2} + \sigma_{j'}^{2}}^{1/2}$. Moreover, on order to tackle this expression, we use the *probabilist's Hermite polynomial* of order $4$: $\phi^{(4)} = \phi(x) H_{4}(x) = \phi(x)*(x^{4} - 6x^{2} + 3)$.

With equations (6.1), (7.1) and (11.1), we obtain the reduced form of the AMISE:

(10.1) $\text{AMISE}[\hat{f}(\cdot;h)] = \frac{1}{4}R(f^{''})h^{4} + \frac{1}{2nh\sqrt{\pi}}$

With optimal bandwidth $h_{\text{AMISE}}$ given by:

(12.1) $h_{\text{AMISE}} = {\left[ {\frac{{(2 \sqrt{\pi})}^{-1}}{R(f^{''})n}} \right]}^{1/5}$

Additionally, under these assumptions, we obtain an explicit and exact MISE expression of equation (8):
...
With this mathematical review, we are able to compare the MISE and AMISE criteria. We code the following functions isn `R`:

* `omega_a()`: Compute the matrix  $(\Omega_{a})_{i,j}$ defined in equation (15).

* `omega()`: Compute the `scalar` given by the following vector and matrix operations of equation (14): $\Omega = \boldsymbol{w}^{'}\{(1 - n^{-1})\Omega_{2} - \Omega_{1} + \Omega_{0}\}\boldsymbol{w}$
...
We consider in particular the following three densities of `nor1mix` whose parameter are available in [R Help](https://marronwebfiles.sites.oasis.unc.edu/OldResearch/parameters/nmpar.m)
...
In the case  using the dataset `nor1mix::MW.nm1` figure \ref{fig:q2plot0} shows the histograms for the sample distribution for $n = 100, 200, 500$. As expected, behavior of the sample improve when we increase the number of observations.
...
Figure \ref{fig:q2plot1} shows the MISE and AMISE related to `nor1mix::MW.nm1`. The values of $h_{\text{AMISE}}$ are `r round(h_AMISE_100,3)`, `r round(h_AMISE_200,3)` and `round(h_AMISE_500,3)`. On the hand values of $h_{\text{MISE}}$ are `r round(h_MISE_100$minimum,3)`, `r round(h_MISE_200$minimum,3)` and `round(h_MISE_500$minimum,3))` for $n = 100, 200, 500$. In the three curves, when we reduce the number of the bandwidth, the MISE and AMISE values are quite similar. Additionally, we can see the effect of increasing the number of samples $n$ in the $y$ axis of the plot. (The scaled is reduced). So, the difference between both

In this case, when we increase the number of samples $n$ and reduce the value of the bandwidth, the curves for MISE and
...
abline(v = h_AMISE_100, col = "blue")
abline(v = h_MISE_100)
text(0.4, 0.06, labels = paste("h_MISE = ", round(h_MISE_100$minimum,3) ), col = "black", cex = 0.6, font = 2, adj = c(0, 0))
text(0.18, 0.06, labels = paste("h_AMISE = ", round(h_AMISE_100,3) ), col = "blue", cex = 0.6, font = 2, adj = c(0, 0))
legend("topright", legend = c("MISE", "AMISE"), lwd = 1, col = c("black", "blue"))
plot(h, MISE_200, type = "l", 
...
abline(v = h_MISE_200)
text(0.4, 0.03, labels = paste("h_MISE = ", round(h_MISE_200$minimum,3) ), col = "black", cex = 0.6, font = 2, adj = c(0, 0))
text(0.15, 0.03, labels = paste("h_AMISE = ", round(h_AMISE_200,3) ), col = "blue", cex = 0.6, font = 2, adj = c(0, 0))
legend("topright", legend = c("MISE", "AMISE"), lwd = 1, col = c("black", "blue"))
plot(h, MISE_500, type = "l", 
...
abline(v = h_AMISE_500, col = "blue")
abline(v = h_MISE_500)
text(0.1, 0.02, labels = paste("h_MISE = ", round(h_MISE_500$minimum,3) ), col = "black", cex = 0.6, font = 2, adj = c(0, 0))
text(0.3, 0.02, labels = paste("h_AMISE = ", round(h_AMISE_500,3) ), col = "blue", cex = 0.6, font = 2, adj = c(0, 0))
legend("topright", legend = c("MISE", "AMISE"), lwd = 1, col = c("black", "blue"))
...
For this question we describe briefly the necessary steps related to the implementation of the wild-bootstrap:
...
2. Bootstrap: In this case, we consider two types of bootstrap:
  ...
  - Wild Bootstrap: This technique is focused on resampling the residuals. Similar to the *residual bootstrap* in which we fixed the covariate $X_{i}$ and generate a new value of $Y_{i}$ using the fitted model and the *noise* from sampling the residuals, additional we generate IID random variables $V_{i}$ (random variable with mean zero and variance 1) and then we can perturb the residuals. This modification improves the stability of the computations, especially in the presence of heteroskedasticity.
